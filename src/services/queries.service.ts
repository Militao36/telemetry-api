...
export class QueriesService {
  ...
  public async reportQueries(
    ...
    const slowesTypeSelect = await this.queriesRepository.slowestQueries(
      idEmpresa,
      hour,
      'select',
      10,
    );
    const slowesTypeInsert = await this.queriesRepository.slowestQueries(
      idEmpresa,
      hour,
      'insert',
      10,
    );
    const slowesTypeUpdate = await this.queriesRepository.slowestQueries(
      idEmpresa,
      hour,
      'update',
      10,
    );
    const slowesTypeDelete = await this.queriesRepository.slowestQueries(
      idEmpresa,
      hour,
      'delete',
      10,
    ...
    const avgQueryTimeByHour = await this.queriesRepository.avgQueryTimeByHour(
      idEmpresa,
      hour,
      queryType,
    );
  ...
  public async dashboardQueries(idEmpresa: string, hour: number) {
    const queriesPerTimeSeries = await this.queriesRepository.getQueriesPerTimeSeries(
      idEmpresa,
      hour,
    );
